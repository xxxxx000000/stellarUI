...
void setup() {
    //pinMode(41, OUTPUT);   // 电池使能引脚，根据不同开发板可替换或者删除 Battery enable pin, can be changed or removed depending on your board
    ...
    screenrotationservice.setRotation(0); // rotation可调节，范围0~3，对应四个方向 Screen rotation can be set from 0 to 3 (four directions)
    ...
    uiManager.pushPage(new MainPage(), TransitionType::None); // 启动之后显示的第一个页面（可更换） The first page to display after startup (can be changed)
