...
public:
    ...
    PopupComponent(float heightRatio_,
               std::function<bool()> visibleLambda,
               std::function<String()> textLambda,
               uint16_t bgColor_ = TFT_WHITE,
               int cornerRadius_ = 8,
    ...
    void draw(TFT_eSprite& buf, int offsetX = 0, int offsetY = 0) override {
        if (useHeightRatio && popupHeight == 0) {
            popupHeight = heightRatio * buf.height();
        }
        width = buf.width() * 0.8f;
        if (!internalBuf) {
