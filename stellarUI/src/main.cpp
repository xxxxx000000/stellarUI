...
void logicTask(void* pvParameters) {
    ...
    const TickType_t xFrequency = pdMS_TO_TICKS(20); // 可根据需要调整执行周期（单位：毫秒）Can be adjusted as needed (unit: milliseconds)
...
void setup() {
    ...
    uiManager.pushPage(new setTimePage(), TransitionType::None); // 启动之后显示的第一个页面（可更换） The first page to display after startup (can be changed)
