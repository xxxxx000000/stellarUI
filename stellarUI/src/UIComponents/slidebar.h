...
public:
    ...
    void draw(TFT_eSprite &buf, int offsetX = 0, int offsetY = 0) override {
        int absX = x + offsetX;
        int absY = y + offsetY;
        buf.fillRoundRect(absX, absY, width, height, radius, backgroundColor);
        ...
        if (direction == SliderDirection::Horizontal) {
            ...
            buf.fillRoundRect(fx, absY, visualFillLen, height, radius, fillColor);
            ...
            buf.fillRoundRect(absX, fy, width, visualFillLen, radius, fillColor);
        }
    }
